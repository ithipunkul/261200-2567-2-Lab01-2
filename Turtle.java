...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] a)
    {
        ...
        for (int i = 0; i < boxSide / fillStep; i++) {
            bob.left(90);
            bob.forward(boxSide);
            bob.right(90);
            bob.up();
            bob.forward(fillStep);
            bob.down();
        ...
        // 2.1 วาดเส้นแนวนอน (พาดผ่านกึ่งกลาง)
        bob.home();
        bob.right(180); // หันไปท
        bob.forward(boxSide / 2); // วาดจากซ้ายไปศูนย์กลาง
        bob.down();
        bob.right(180); // หันกลับไปทางขวา
        bob.forward(boxSide*2); // วาดจากซ้ายไปขวา

        // 2.2 วาดเส้นแนวตั้ง (พาดผ่านกึ่งกลาง)
        bob.up();
        bob.home();
        bob.left(90); // หันไปทางขึ้น
        bob.forward(boxSide / 2); // วาดจากศูนย์กลางขึ้นไป
        bob.down();
        bob.right(180); // หันกลับไปทางลง
        bob.forward(boxSide); // วาดจากบนลงล่าง
        ...
        bob.right(180); // หันกลับไปทางลง
        bob.forward(boxSide); // วาดจากบนลงล่าง
        // --- ส่วนที่ 3: โค้ดวาดโบว์ ---

        bob.up();
        bob.home();
        int bowSize = 30; // ความยาวของแฉกโบว์
        int offset = boxSide / 2; // ระยะครึ่งหนึ่งของกล่อง (50)

        bob.penColor("pink"); // ใช้สีทองสำหรับโบว์

// 3.1 ย้ายไปที่ตำแหน่งศูนย์กลางด้านบนของกล่อง
// กล่อง 100x100 มีขอบด้านบนที่ Y = 50
        bob.forward(-50);

        bob.left(90); // หันขึ้น
        bob.forward(55); // เลื่อนไปที่กึ่งกลางขอบบน (Y=50)

// 3.2 วาดปมโบว์ 4 แฉก (Knot/Star)
        bob.down();
        for (int i = 0; i < 8; i++) {
            // วาด 1 แฉก (ไปแล้วกลับ)
            bob.forward(bowSize);
            bob.backward(bowSize);

            // หมุน 90 องศาเพื่อวาดแฉกถัดไป
            bob.right(45);
        }


        bob.up();
        bob.home(); // กลับสู่จุดเริ่มต้น
    }
